...
async def main():
    ...
    async with OKXClient() as client:
        ...
        if candles:
            print(
                f"  First: {candles[0].open_time} -> O={candles[0].open} H={candles[0].high} L={candles[0].low} C={candles[0].close}"
            )
    ...
    async for session in get_session():
        ...
        count_before = await repo.count_in_range(
            "XAU-USDT-SWAP",
            "15m",
        ...
        count_after = await repo.count_in_range(
            "XAU-USDT-SWAP",
            "15m",
            datetime(2026, 7, 1, tzinfo=UTC),
            datetime(2026, 7, 16, tzinfo=UTC),
        ...
        if count_after > 0:
            ...
            for r in results[:3]:
                print(
                    f"  ts={r.timestamp} open={r.open_price} high={r.high} low={r.low} close={r.close}"
                )
